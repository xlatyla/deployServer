...
if [ "$HORA_ACTUAL" -lt 7 ] || [ "$HORA_ACTUAL" -ge 20 ]; then
    ...
    /usr/bin/docker stop xpo-report-app prelist-report-app veronelli-app ips-mail-app error-interface-app servicio_impresion-app omya-sftp-app ntl-ftp-app barcelonesa-app > /dev/null 2>&1
else
